...
const vec3 ENV_POLE = vec3(0.015, 0.035, 0.020);
const vec3 ENV_HORIZON = vec3(0.070, 0.170, 0.090);

vec3 environment(vec3 r)
{
    float h = 1.0 - abs(r.z);
    return mix(ENV_POLE, ENV_HORIZON, h * h);
}
...
void main()
{
    ...
    vec3 color = min(highlight * blobSpecColor + environment(normalize(fragEnvRefl)), 1.0);
